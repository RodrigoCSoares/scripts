...
-spec set_header(request(ELP), binary(), binary()) -> request(ELP).
...
-spec prepend_header(request(ELS), binary(), binary()) -> request(ELS).
...
-spec set_body(request(any()), ELX) -> request(ELX).
set_body(Req, Body) ->
    {request,
...
-spec map(request(ELZ), fun((ELZ) -> EMB)) -> request(EMB).
...
-spec set_query(request(EML), list({binary(), binary()})) -> request(EML).
set_query(Req, Query) ->
    Query@1 = begin
...
-spec set_method(request(EMP), gleam@http:method()) -> request(EMP).
...
-spec set_scheme(request(EMW), gleam@http:scheme()) -> request(EMW).
...
-spec set_host(request(EMZ), binary()) -> request(EMZ).
set_host(Req, Host) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_port(request(ENC), integer()) -> request(ENC).
set_port(Req, Port) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_path(request(ENF), binary()) -> request(ENF).
set_path(Req, Path) ->
    {request,
        erlang:element(2, Req),
        erlang:element(3, Req),
...
-spec set_cookie(request(ENI), binary(), binary()) -> request(ENI).
...
-spec remove_cookie(request(ENO), binary()) -> request(ENO).
