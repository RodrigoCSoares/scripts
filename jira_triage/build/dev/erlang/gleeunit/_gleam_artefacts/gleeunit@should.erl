...
-spec be_ok({ok, FMG} | {error, any()}) -> FMG.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FML}) -> FML.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FMO)) -> FMO.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
