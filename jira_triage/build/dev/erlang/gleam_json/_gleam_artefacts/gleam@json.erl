...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FGI)) -> {ok, FGI} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FGC)) -> {ok, FGC} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FFY)) -> {ok, FFY} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FGO), fun((FGO) -> json())) -> json().
...
-spec array(list(FGS), fun((FGS) -> json())) -> json().
