...
-opaque subject(DPN) :: {subject, pid_(), gleam@dynamic:dynamic_()} |
    {named_subject, name(DPN)}.

-type name(DPO) :: any() | {gleam_phantom, DPO}.

-type do_not_leak() :: any().

-type selector(DPP) :: any() | {gleam_phantom, DPP}.
...
-spec 'receive'(subject(DQN), integer()) -> {ok, DQN} | {error, nil}.
...
-spec receive_forever(subject(DQV)) -> DQV.
...
-spec selector_receive(selector(DQZ), integer()) -> {ok, DQZ} | {error, nil}.
...
-spec selector_receive_forever(selector(DRD)) -> DRD.
...
-spec map_selector(selector(DRF), fun((DRF) -> DRH)) -> selector(DRH).
map_selector(A, B) ->
...
-spec merge_selector(selector(DRJ), selector(DRJ)) -> selector(DRJ).
merge_selector(A, B) ->
...
-spec select_trapped_exits(selector(DRN), fun((exit_message()) -> DRN)) -> selector(DRN).
...
-spec select_map(selector(DRU), subject(DRW), fun((DRW) -> DRU)) -> selector(DRU).
...
-spec select(selector(DRQ), subject(DRQ)) -> selector(DRQ).
...
-spec deselect(selector(DRZ), subject(any())) -> selector(DRZ).
...
-spec select_record(
    ...
    any(),
    integer(),
    fun((gleam@dynamic:dynamic_()) -> DSE)
) -> selector(DSE).
...
-spec select_other(selector(DSI), fun((gleam@dynamic:dynamic_()) -> DSI)) -> selector(DSI).
...
-spec select_specific_monitor(selector(DSU), monitor(), fun((down()) -> DSU)) -> selector(DSU).
...
-spec select_monitors(selector(DSX), fun((down()) -> DSX)) -> selector(DSX).
...
-spec deselect_specific_monitor(selector(DTA), monitor()) -> selector(DTA).
...
-spec perform_call(
    subject(DTD),
    fun((subject(DTF)) -> DTD),
    fun((selector(DTF)) -> {ok, DTF} | {error, nil})
) -> DTF.
...
-spec call(subject(DTK), integer(), fun((subject(DTM)) -> DTK)) -> DTM.
...
-spec call_forever(subject(DTO), fun((subject(DTQ)) -> DTO)) -> DTQ.
...
-spec send_after(subject(DTW), integer(), DTW) -> timer().
