...
-file("src/jira.gleam", 104).
-spec check_api_error(binary()) -> {ok, nil} | {error, binary()}.
check_api_error(Raw_file) ->
    case shellout:command(
        <<"jq"/utf8>>,
        [<<"-e"/utf8>>, <<".issues"/utf8>>, Raw_file],
        <<"."/utf8>>,
        []
    ) of
        {ok, _} ->
        ...
        {error, _} ->
            Msg = shellout:command(
                <<"jq"/utf8>>,
                [<<"-r"/utf8>>,
                    <<"(.errorMessages // [\"Unknown error\"])[0] // .message // \"Check your JIRA_API_TOKEN and JIRA_EMAIL\""/utf8>>,
                    Raw_file],
                <<"."/utf8>>,
                []
            ),
            case Msg of
                {ok, M} ->
                    {error,
                        <<"Jira API error: "/utf8,
                            (gleam@string:trim(M))/binary>>};

                {error, _} ->
                    {error, <<"Jira API error: check your credentials"/utf8>>}
            end
    end.

-file("src/jira.gleam", 22).
