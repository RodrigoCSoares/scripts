...
-spec map_response_body(
    fun((EZF) -> gleam@http@response:response(EZG)),
    fun((EZG) -> EZI)
) -> fun((EZF) -> gleam@http@response:response(EZI)).
...
-spec prepend_response_header(
    fun((EZK) -> gleam@http@response:response(EZL)),
    binary(),
    binary()
) -> fun((EZK) -> gleam@http@response:response(EZL)).
...
-spec ensure_post(gleam@http@request:request(EZO)) -> {ok,
        gleam@http@request:request(EZO)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(EZX)) -> EZZ)) -> fun((gleam@http@request:request(EZX)) -> EZZ).
