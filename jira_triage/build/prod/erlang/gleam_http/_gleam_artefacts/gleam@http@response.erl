...
-type response(EVJ) :: {response, integer(), list({binary(), binary()}), EVJ}.

-file("src/gleam/http/response.gleam", 87).
?DOC(" Set the body of the response, overwriting any existing body.\n").
-spec set_body(response(any()), EWG) -> response(EWG).
...
-spec try_map(response(EVK), fun((EVK) -> {ok, EVM} | {error, EVN})) -> {ok,
        response(EVM)} |
    {error, EVN}.
...
-spec set_header(response(EVY), binary(), binary()) -> response(EVY).
...
-spec prepend_header(response(EWB), binary(), binary()) -> response(EWB).
...
-spec map(response(EWI), fun((EWI) -> EWK)) -> response(EWK).
...
-spec expire_cookie(response(EWT), binary(), gleam@http@cookie:attributes()) -> response(EWT).
